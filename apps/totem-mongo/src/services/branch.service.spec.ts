...
import {
  NullBranchIdException, BranchCreateException, BranchInterneErrorException,
  BranchNotFoundException, InvalidBranchPayloadException,
...
describe('BranchService', () => {
  ...
  beforeEach(async () => {
    const mockBranchModel = {
      find: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue([mockBranchDocument]) }),
      findById: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(mockBranchDocument) }),
      findByIdAndUpdate: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(mockBranchDocument) }),
  ...
  it('Liste => OK', async () => {
    ...
    expect(branchModel.find).toHaveBeenCalledWith({ is_deleted: { $ne: true } });
  });

  it('GetID => OK', async () => {
    const result = await service.getById(mockBranchDocument._id.toString());
    expect(result).toEqual(mockBranchDocument);
    expect(branchModel.findById).toHaveBeenCalledWith(mockBranchDocument._id.toString());
  });
  ...
  it('GetID => Exception: Branch not Found', async () => {
    ...
    await expect(service.getById("111111111111111111111111")).rejects.toThrow(BranchNotFoundException);
  });

  it('GetID => Exception: ID null', async () => {
    await expect(service.remove(undefined as any)).rejects.toThrow(NullBranchIdException);
  });
  ...
  it('Create => Exception: date invalide', async () => {
    ...
    await expect(service.create(invalidDto)).rejects.toThrow(BranchCreateException);
  });
  ...
  it('Update => Exception: ID null', async () => {
    ...
    await expect(service.update(null as any, updateDto)).rejects.toThrow(NullBranchIdException);
  });
  ...
  it('Delete => Exception: Branch not found', async () => {
    ...
    await expect(service.remove('111111111111111111111111')).rejects.toThrow(BranchNotFoundException);
  });
