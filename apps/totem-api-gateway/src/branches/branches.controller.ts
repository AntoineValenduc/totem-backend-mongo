...
export class BranchesController {
  ...
  constructor(
    private readonly brancheService: BranchesService
  ) {}

  @Get()
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Lister tous les branches' })
  @ApiResponse({ status: 200, description: 'Liste des branches', type: [BrancheCreateDto] })
  ...
  @ApiResponse({ status: 200, description: 'Branche trouvé', type: BrancheCreateDto })
  @ApiResponse({ status: 400, description: 'ID invalide'})
  @ApiResponse({ status: 404, description: 'Branche introuvable' })
  @ApiResponse({ status: 500, description: 'Erreur interne du serveur' })
  getById(@Param('id') id: string) {
    this.logger.log("✅ Requête envoyé => getById branche MongoDB, ID : ", { id });
  ...
  @ApiResponse({ status: 201, description: 'Branche créé avec succès', type: BrancheCreateDto })
  ...
  @ApiResponse({ status: 200, description: 'Branche mis à jour avec succès', type: BrancheCreateDto })
  @ApiResponse({ status: 400, description: 'Données invalides' })
  @ApiResponse({ status: 404, description: 'Branche introuvable' })
  @ApiResponse({ status: 500, description: 'Erreur interne du serveur' })
  updateBranch(@Param('id') id: string, @Body() branch: BrancheUpdateDto) {
    this.logger.log("✅ Requête envoyée => update branche, ID : ", { id, branch });
